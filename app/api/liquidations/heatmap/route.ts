...
const RANGES = ['12h', '24h', '3d', '7d', '30d', '90d', '180d', '1y'] as const;

export async function GET(req: NextRequest) {
  return handle(async () => {
    checkRateLimit(req);
    // Both values are interpolated into an upstream request URL, so both are
    // validated rather than passed through.
    const { coin, range } = parseQuery(req, z.object({
      coin: symbolSchema.default('BTC'),
      range: z.enum(RANGES).default('24h'),
    }));

    const symbol = toCanonical(coin.replace('USDT', ''));
    const res = await cached(`liqheat:${symbol}:${range}`, TTL.liquidation, async () => {
      ...
      if (coinglassConfigured()) {
        ...
        if (cg.configured && cg.available) {
          return {
            data: {
              source: 'coinglass' as const, currentPrice: price, heatmap: cg.data,
              note: `Live CoinGlass liquidation heatmap (${DEFAULT_EXCHANGE}, ${range}).`,
            },
            sources, errors,
          };
        }
        // Configured but unavailable: say why, then fall back to the clearly
        // labelled estimate rather than showing nothing.
        const reason = 'message' in cg ? cg.message : 'unavailable';
        return {
          data: {
            source: 'estimated' as const, currentPrice: price,
            column: estimateHeatmapColumn(price, oi),
            note: `CoinGlass configured but heatmap unavailable (${reason}). The pair Heatmap `
              + 'endpoint needs a Professional/Enterprise plan. Showing exchange-derived ESTIMATE '
              + '(current snapshot).',
          },
          sources, errors,
        };
      ...
      return {
        data: {
          source: 'estimated' as const, currentPrice: price,
          column: estimateHeatmapColumn(price, oi),
          note: 'CoinGlass not configured — exchange-derived ESTIMATE (current snapshot, not a '
            + 'time series). Set COINGLASS_API_KEY (Professional+ plan) for the real heatmap.',
        },
        sources, errors,
      };
    });

    return envelope(res.data, res.sources, res.errors);
  });
}
