...
export default function LoginPage() {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    if (!sb) {
      setMsg("Chưa cấu hình Supabase (xem README).");
      return;
    }
    setLoading(true);
    setMsg(null);
    ...
    if (mode === "signup" && !res.data.session) {
      setMsg("Đã gửi email xác nhận. Vui lòng kiểm tra hộp thư rồi đăng nhập.");
      return;
    }
    router.push("/quan-ly");
    router.refresh();
  ...
  async function handleGoogle() {
    ...
    await sb.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/quan-ly` }
    });
  }
        ...
        <CardContent className="pt-6">
          {!configured && (
            <div className="mb-4 flex items-start gap-2 rounded-lg bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
              ...
              <span>
                Chưa cấu hình Supabase. Web đang chạy chế độ cục bộ (lưu trong máy).
                Thêm 2 biến môi trường (xem README) để bật đăng nhập & lưu đám mây.
              </span>
            </div>
          )}
          ...
          <form onSubmit={handleEmail} className="space-y-3">
            ...
            <div className="relative">
              ...
              <Input type="password" required placeholder="Mật khẩu" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-9" />
            </div>
